...
class Picarx(object):
    ...
    def backward(self, speed):
        ...
        if current_angle != 0:
            ...
            l = 95
            d = 110
            x = l / (np.tan(math.radians(current_angle)))

            w = (speed * np.tan(math.radians(current_angle)))/ l
            v_right = w * (x - (d/2)) # motor 2
            v_left = w * (x + (d/2)) # motor 1

            if (current_angle / abs_current_angle) > 0:
                self.set_motor_speed(1, -1*v_left)
                self.set_motor_speed(2, v_right)
    ...
    def forward(self, speed):
        ...
        if current_angle != 0:
            ...
            if (current_angle / abs_current_angle) > 0:
                ...
                self.set_motor_speed(1, v_left)
                #self.set_motor_speed(2, -1*speed * power_scale)
                self.set_motor_speed(2, -1*v_right)
        else:
            self.set_motor_speed(1, speed)
